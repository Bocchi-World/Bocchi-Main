...
--// PERFORMANCE: enable lightweight mode to reduce lag (can be toggled at runtime)
Starlight.PerformanceMode = true  -- set to false to enable full visual effects
...
local function BlurModule(Frame : Frame)
	-- Performance guard: no-op in PerformanceMode to avoid per-frame RenderStep and DOF clones
	if Starlight and Starlight.PerformanceMode then
		local stub = {}
		function stub.updateAll() end
...
local function AddToolTip(InfoStr, HoverInstance)
	...
	tooltip.Name = (HoverInstance and HoverInstance.Name) or ("Tooltip_"..HttpService:GenerateGUID(false))
	...
	local function updateTooltipPos(mx, my)
		tooltip.Position = UDim2.fromOffset((mx or Mouse.X) + 15, (my or Mouse.Y) + 20)
	end
	...
	if HoverInstance then
		...
		-- Use Mouse.Move instead of a per-frame loop
...
do
	...
	btn.Position = UDim2.fromOffset(10, (GuiInset > 0 and GuiInset or 10))
	btn.BackgroundColor3 = Color3.fromRGB(27,29,33)
	btn.Image = "rbxassetid://3926305904" -- uses same style as tab icons
	btn.ImageColor3 = Color3.fromRGB(200,200,200)
	btn.AutoButtonColor = false
	btn.ZIndex = 500
	btn.Visible = UserInputService.TouchEnabled -- only show on touch devices by default
	local uiCorner = Instance.new("UICorner", btn); uiCorner.CornerRadius = UDim.new(0,6)
	local stroke = Instance.new("UIStroke", btn); stroke.Transparency = 0.6
	btn.Parent = StarlightUI

	local function setIcon(open)
		btn.Rotation = open and 0 or 0
		btn.ImageTransparency = 0
	end
	setIcon(true)

	btn.MouseButton1Click:Connect(function()
		local open = not Starlight.Minimized and StarlightUI.MainWindow.Visible
		if open then
			StarlightUI.MainWindow.Visible = false
			Starlight.Minimized = true
		else
			StarlightUI.MainWindow.Visible = true
			Starlight.Minimized = false
			-- ensure window is on-screen
			StarlightUI.MainWindow.Position = UDim2.fromOffset(
				math.max(1, math.min(Camera.ViewportSize.X - StarlightUI.MainWindow.Size.X.Offset - 1, StarlightUI.MainWindow.Position.X.Offset)),
				math.max(1, math.min(Camera.ViewportSize.Y - StarlightUI.MainWindow.Size.Y.Offset - 1, StarlightUI.MainWindow.Position.Y.Offset))
			)
		end
	end)
end
...
-- Ensure non-interactive overlays don't block input (right-edge ghost hitbox fix)
pcall(function()
	for _, container in ipairs({StarlightUI:FindFirstChild("Popup Overlay"), mainWindow:FindFirstChild("DropShadowHolder"), mainWindow:FindFirstChild("Content")}) do
		if typeof(container) == "Instance" and container then
			...
			for _, obj in ipairs(container:GetDescendants()) do
				if obj:IsA("GuiObject") then
					if obj.Name:match("^Fade") or obj.Name:match("^FADE_") or obj.Name:match("Shadow") then
						obj.Active = false
						obj.ClipsDescendants = true
					end
					if obj.Name:match("Overlay") then
						obj.Active = false
					end
				end
			end
